...
import "../style/RecruiterHome.scss";
import "../style/CandidateHome.scss";
...
import PersonalInfoEdit from "../components/RecruiterDashboard/EditModals/PersonalInfoEdit";
import AddEmployment from "../components/RecruiterDashboard/EditModals/AddEmployment";
import AddEducation from "../components/RecruiterDashboard/EditModals/AddEducation";
import AddSkillPreferences from "../components/RecruiterDashboard/EditModals/AddSkillPreferences";
import { setSnackbar } from "../../redux/flags/flagActions";
import Link from "next/link";
import { getAge } from "../utils/Functions";

import EditProfile from "../components/RecruiterDashboard/EditModals/EditProfile";
import CompanyInfoEdit from "../components/RecruiterDashboard/EditModals/CompanyInfoEdit";
import EditProfileImage from "../components/RecruiterDashboard/EditModals/EditProfileImage";
...
function RecruiterHome(props) {
  ...
  const [open, setOpen] = useState(false); //personal info open trigger
  ...
  const [companyImgFlag, setCompanyImgFlag] = useState(false);
  const [flag, setFlag] = useState(false);

  const [key, setKey] = useState(""); //key to pass to personal info prop
  const [keyName, setKeyName] = useState("");
  const [editData, setEditData] = useState(null); //edit data to pass to the edit respective edit component
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/getRecruiterProfile`,
  ...
  const upload2 = (e) => {
    ...
    if (!Array.isArray(e.target.files[0])) {
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/upload-recruiter-data`,
          formdata,
          {
            headers: {
        ...
        .then((res) => {
          ...
          props.setSnackbar({
            type: "success",
            text: "GSTIN Updated Successfully",
            open: true,
          });
          setFlag(!flag);
        })
        ...
        >
          ...
          {props.user.userInfo && (
            <>
              ...
              <CompanyInfoEdit open={open6} setOpen={setOpen6} />
              <EditProfileImage
                open={open7}
                setOpen={setOpen7}
              ...
              <section className="candidate-info-section row m-auto shadow-sm">
                ...
                <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 info-cont">
                  ...
                  <p className="grey-text">
                    {userInfo.gender} |{" "}
                    {userInfo.dob ? getAge(userInfo.dob) : "DOB Missing"}
                  </p>
                  <div className="row m-auto sub-info">
                    <p className="mx-2">
                      <LocalPhoneIcon sx={{ marginRight: 0.1 }} />{" "}
                      <b>{userInfo.mobileNo}</b>
                    </p>
                    <p className="mx-2">
                      <AlternateEmailIcon sx={{ marginRight: 0.1 }} />{" "}
                      <b>{userInfo.email}</b>
                    </p>
                    <p className="mx-2">
                      <LocationOnIcon sx={{ marginRight: 0.1 }} />{" "}
                      <b>
                        {userInfo.userLocation.city},{" "}
                        {userInfo.userLocation.state}
                      </b>
                    </p>
                  </div>
                  <p className="resume-tagline grey-text">
                    {userInfo.resumeTagline
                      ? userInfo.resumeTagline
                      : "Tagline Missing"}
                  </p>
                ...
                <div className="col-12 col-sm-12 col-md-4 col-lg-4 col-xl-4 resume-div">
                  <div className="resume-head shadow-sm">
                    {userInfo.gstin ? (
                      <div className="resume-child row m-auto">
                        ...
                        <div
                          className="col-8 underline-hover"
                          onClick={() =>
                            window.open(
                              `${process.env.REACT_APP_DEVELOPMENT}/api/pdf/${userInfo.gstin}`,
                              "_blank"
                            )
                          }
                        >
                          <h4>
                            {userInfo.fullName}&apos;s GSTIN.
                            {userInfo.gstin.split(".")[1]}
                          </h4>
                    ...
                    {userInfo.pan ? (
                      <div className="resume-child row m-auto">
                        ...
                        <div
                          ...
                          onClick={() =>
                            window.open(
                              `${process.env.REACT_APP_DEVELOPMENT}/api/pdf/${userInfo.pan}`,
                              "_blank"
                            )
                          }
                        >
                          <h4>
                            {userInfo.fullName}&apos;s Pan.
                            {userInfo.pan.split(".")[1]}
                          </h4>
                  ...
                  <div className="row m-auto subs-div align-items-center justify-content-around">
                    <div className="col-8">
                      ...
                      <p>
                        Upgrade your plan for more job posting&apos;s and resume
                        access
                      </p>
                    </div>
                    <div className="col-4">
              ...
              <section className="company-info-section shadow-sm row m-auto">
                <div className="col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
                  <img
                    src={
                      props.user.userInfo.companyImg
                        ? `${process.env.REACT_APP_DEVELOPMENT}/api/image/${props.user.userInfo.companyImg}`
                        : "/user.png"
                    }
                    alt="profile"
                  />
                  {!userInfo.subRecruiter && (
                    <div className="mt-4 mb-2" style={{ textAlign: "center" }}>
                  ...
                  <div className="col-11">
                    ...
                    <div className="row m-auto">
                      <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 key-holders">
                        <p>
                          <LocationOnIcon /> {userInfo.companyLocation.city},{" "}
                          {userInfo.companyLocation.state}
                        </p>
                      </div>
                      <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 key-holders">
                        <Link
                          href={{ pathname: userInfo.companyUrl }}
                          target="_blank"
                        >
                          <p>
                            <LinkIcon /> {userInfo.companyUrl}
                          </p>
                        </Link>
                      </div>
                    </div>
                    <div className="mt-4" />
                    {userInfo.companyTags.map((item, index) => (
                      <Chip
                        key={index}
                        color="primary"
                        className="m-2"
                        ...
                        (isNaN(
                          Math.ceil(
                            (dashboardData.hiredCandidates[0].total /
                              (dashboardData.hiredCandidates[0].total +
                                dashboardData.pendingCandidates[0].total +
                                dashboardData.rejectedCandidates[0].total)) *
                              100
                          )
                        )
                          ? 0
              ...
              <section className="row m-auto info-sections justify-content-around">
                <div
                  className={
                    !userInfo.subRecruiter
                      ? "col-12 col-sm-12 col-md-5 col-lg-5 col-xl-5 info-child shadow-sm"
                      : "col-12 info-child shadow-sm"
                  }
                >
                  ...
                  <div className="row m-auto justify-content-between">
                    ...
                    <div>{/* <Button>Add Details</Button> */}</div>
                  </div>
                  ...
                  <div className="col-12 col-sm-12 col-md-5 col-lg-5 col-xl-5 info-child shadow-sm">
                    ...
                    {userInfo.workExperience.length > 0 ? (
                      userInfo.workExperience.map((item, index) => (
                        <div
                          key={index}
                          className="row m-auto justify-content-between info-sub-content"
                        >
                          ...
                          >
                            ...
                            <IconButton
                              onClick={() => {
                                axios
                                  .post(
                                    `${process.env.REACT_APP_DEVELOPMENT}/api/recruiter/removeEmployment`,
                                    { obj: item },
                                    { headers: { token: props.user.user } }
                                  )
                                  .then((res) => {
