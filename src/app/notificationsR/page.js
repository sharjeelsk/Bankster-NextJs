...
import "../style/NotificationsR.scss";
...
function Notifications(props) {
  ...
  const handleInfo = (id, schema) => {
    ...
    } else if (schema === "subscription") {
      setSingleData({ data: "", schema: "subscription" });
    }
  };
  ...
  const renderData = () => {
    if (singleData.schema === "job") {
      return (
        <div>
          ...
          <div className="row my-2 mx-auto key-features">
            <div className="m-1">
              ...
              <span className="key-headline m-2">
                {singleData.data.experience.min} -{" "}
                {singleData.data.experience.max} Yrs
              </span>
            </div>
            <div className="m-1">
              <CurrencyRupeeIcon color="primary" />
              <span className="key-headline m-2">
                {singleData.data.ctc.min} - {singleData.data.ctc.max} P.A
              </span>
            </div>
            <div className="m-1">
              <FmdGoodIcon color="primary" />
              <span className="key-headline m-2">
                {singleData.data.jobLocation.city} |{" "}
                {singleData.data.jobLocation.state} |{" "}
                {singleData.data.jobLocation.country}
              </span>
            </div>
          </div>
          <hr />
          ...
          <hr />
          <Button
            onClick={() => router.push(`/jobdetail/${singleData.data._id}`)}
            variant="outlined"
          >
            View Job
          </Button>
    ...
    } else if (singleData.schema === "subscription") {
      return (
        <div>
          <h3>View Subscription From the link below</h3>
          {/* <Button onClick={()=>router.push("/subscriptionr")}>Go to subscription</Button> */}
          <hr />
          <Button
  ...
  const handleDelete = () => {
    ...
    axios
      .post(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/deleteNotification`,
        { object: activeObj },
            ...
            <>
              ...
              {render && (
                <div className="mobile-visible">
                  <IconButton onClick={() => setRender(false)}>
                    <ArrowBackIcon />
                  </IconButton>
                </div>
              )}
              <section className="notification-head row m-auto justify-content-around">
                <section
                  className={`notification-container col-12 col-sm-12 col-md-7 col-lg-7 col-xl-7 p-0 ${
                    render ? "display-false" : "display-true"
                  }`}
                >
                  {props.user.userInfo.notifications.map((item, index) => (
                    <div
                      onClick={() => {
                        setActiveObj(item);
                        handleInfo(item._id, item.schema);
                      }}
                      key={index}
                      className={`row m-auto notification-parent align-items-center ${
                        activeObj &&
                        activeObj.createdAt === item.createdAt &&
                        "active-noti"
                      }`}
                    >
                      <div className="col-2">
                        <img src="/notification-bell.png" alt="logo1" />
                      </div>
                      <div className="col-10">
                        <h3>{item.title}</h3>
                        <p>
                          2h <FiberManualRecordIcon sx={{ fontSize: 8 }} />{" "}
                          {item.description}
                        </p>
                      </div>
                    </div>
                ...
                <section
                  className={`notification-detail col-12 col-sm-12 col-md-4 col-lg-4 col-xl-4 ${
                    render ? "display-true" : "display-false"
                  }`}
                >
                  {renderData()}
                </section>
