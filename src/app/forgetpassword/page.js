...
const ForgetPassword = (props) => {
  ...
  const handlePasswordReset = () => {
    ...
    if (values.password !== values.confirmPassword) {
      ...
      if (props.location.search.split("?")[1] === "candidate") {
        ...
        link = `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/candidate/resetPassword`;
      } else {
        console.log("recrter");
        link = `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/resetPassword`;
      }
      axios
        .post(link, {
          email: email,
  ...
  const handleOtpSend = () => {
    ...
    if (email.length > 0) {
      if (props.location.search.split("?")[1] === "candidate") {
        console.log("candidate");
        link = `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/candidate/sendOTP`;
      } else {
        console.log("recrter");
        link = `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/sendOTP`;
      }
      axios
        .post(link, {
          email: email,
  ...
  const onSubmit = (data) => {
    ...
    let link;
    if (props.location.search.split("?")[1] === "candidate") {
      link = `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/candidate/verifyOTP`;
    } else {
      link = `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/verifyOTP`;
    }
    axios
      .post(link, {
        email: email,
                  ...
                  >
                    ...
                    {!resetPassword ? (
                      <>
                        ...
                        />
                        {error.length > 0 && (
                          <Alert className="alert my-3" severity="error">
                            {error}
                          </Alert>
                        )}
                        <Button
