  ...
  return (
    <div>
      <section
        className={
          props.fullWidth
            ? "shadow searchbar-container row m-auto align-items-center width-full"
            : "shadow searchbar-container row m-auto align-items-center"
        }
      >
        <div
          className={
            props.fullWidth
              ? "col-12 col-sm-12 col-md-4 col-lg-4 col-xl-4 searchbarinputbox"
              : "col-12 col-sm-12 col-md-4 col-lg-4 col-xl-4 searchbarinputbox"
          }
        >
          <input
            className="mt-3"
            onChange={(e) =>
              setSearchObj({ ...searchObj, title: e.target.value })
            }
            placeholder="Search Job By Keyword eg. (Manager)"
          />
        ...
        >
          <Link
            href={`/findjobs?title=${searchObj.title}&location=${searchObj.location}`}
          >
            <Button startIcon={<SearchIcon />} fullWidth variant="contained">
              Search
            </Button>
          </Link>
        </div>
