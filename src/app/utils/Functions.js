...
export const stringHide = (string, type, props) => {
  if (props.hide && type === "fullName") {
    let finalString = `**** ${
      string.includes(" ") ? string.split(" ")[1] : ""
    }`;
    return finalString.length > 15
      ? `${finalString.substring(0, 15)} ...`
      : finalString;
  ...
  if (props.fresher) {
    ...
    if (strobj) {
      ...
      if (strobj.designation) {
        return `${
          strobj.designation.length > 15
            ? `${strobj.designation.substring(0, 15)}..`
            : strobj.designation
        } | ${
          strobj.name.length > 15
            ? `${strobj.name.substring(0, 15)}..`
            : strobj.name
        }`;
      } else {
        return "";
...
export const renderEmployementStringHide = (props) => {
  if (props.fresher) {
    ...
    if (strobj) {
      if (strobj.designation) {
        return `${limitString(
          strobj.designation.substr(0, strobj.designation.length - 3)
        )}*** | ${limitString(
