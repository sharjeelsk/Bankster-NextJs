...
function Bookmark(props) {
  ...
  const fetchBookmarks = () => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/getAllBookmarkedJobs`,
        ...
        >
          ...
          {candidates.length > 0 ? (
              ...
              <section className="candidate-single-job shadow-sm" key={index}>
                <div className="row m-auto">
                  <div className="col-3 col-sm-1 col-md-1 col-lg-1 col-xl-1">
                    <img
                      src={`${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/image/${item.profilePicture}`}
                      alt="logo1"
                    />
                  </div>
                  <div className="col-9 col-sm-5 col-md-5 col-lg-5 col-xl-5">
                    ...
                    <p className="grey-text">
                      {item.gender} |{" "}
                      {item.dob ? getAge(item.dob) : "DOB Missing"}
                    </p>
                    <p className="grey-text">
                      {item.yearsOfExperience
                        ...
                        : "Not added"}{" "}
                      Years of Experience |{" "}
                      {item.currentCtc ? item.currentCtc : "Not added"} CTC |{" "}
                      {item.product ? item.product : "Not added"} |{" "}
                      {item.noticePeriod ? item.noticePeriod : "Not added"}
                    </p>
                    ...
                    <p className="bold-text">
                      {item.education.length > 0
                        ? `${item.education[0].name}, ${item.education[0].universityName}`
                        : ""}
                    </p>
                  </div>
                  <div className="col-10 col-sm-10 col-md-5 col-lg-5 col-xl-5">
                    {item.resume.length > 0 && (
                      <div
                        onClick={() => {
                          window.open(
                            `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/pdf/${item.resume}`,
                            "_blank"
                          );
                          axios
                            .post(
                          ...
                          //window.location.href=`${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/pdf/${item.resume}`
                ...
                <div className="row mt-4 mx-auto sub-info">
                  ...
                  <p className="mx-2">
                    ...
                    <b>
                      {item.userLocation.city}, {item.userLocation.state}
                    </b>
                  </p>
                </div>
                <p className="tagline">{item.resumeTagline}</p>
                {item.skills.map((i, index) => (
                  <Chip label={i} key={index} color="primary" className="m-2" />
                ))}
              </section>
            ))
          ) : (
            <div className="col-12 no-jobs">
              ...
              {/* <Button onClick={()=>router.push("/findjobs")} startIcon={<SearchIcon />} variant="contained">bookmark candidates</Button> */}
            </div>
          )}
        </div>
      </div>
