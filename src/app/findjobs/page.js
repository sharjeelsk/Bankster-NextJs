...
function FindJobs(props) {
  ...
  const [limit, setLimit] = useState({ req1: 20, req2: 20, req3: 20 });
  ...
  let title = props.searchParams.title;
  let location = props.searchParams.location;
  let salary = props.searchParams.salary;
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/getAllJobLocations`
      )
      .then((res) => {
        console.log(res);
    ...
    if (title || location || salary) {
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/searchJobs`,
          {
            title: title ? title : "",
          ...
          },
          {
            headers: {
              token:
                "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InRyaXZlc2hAYmFua3N0ZXIuY29tIiwiX2lkIjoiNjJmM2E3ZjA0ODA4OWE4MDFkM2E3ODA3IiwiaWF0IjoxNjYxMTUyOTUwfQ.yI7xfT8AUAs4NM1S3xsw5xnttnr-cYmHdty0r_itRes",
            },
          }
        )
        .then((res) => {
    ...
    } else if (props.searchParams.key) {
      console.log("inside location");
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/searchKeyJob`,
          {
            ...props.searchParams,
            limit: limit.req2,
          }
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/getAllJobs?sort=${sort}`,
          { limit: limit.req3 }
        )
        .then((res) => {
          console.log(res);
  ...
  const findJob = (type, value) => {
    if (type === "companyImg") {
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/searchJobViaCompany`,
          { companyName: value }
        )
        .then((res) => {
          console.log(res);
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/searchJobViaTag`,
          {
            tags: [value],
          }
        )
    ...
    <div>
      ...
      <section className="row m-auto find-jobs-head">
        <div className="p-0 col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
          <div className="display-mobile">
            {!display ? (
              ...
              <FilterMenu setJobs={setJobs} setDisplay={setDisplay} />
            )}
          </div>
        ...
        <div className="col-12 col-sm-12 col-md-7 col-lg-7 col-xl-7 find-jobs-content">
          <SearchBar
            totalLocations={
              jobData !== null ? jobData.map((item) => item._id) : []
            }
            fullWidth={true}
          />
          <div className="mt-4">
            ...
            <Button className="ml-auto" onClick={() => getAllJobs()}>
              All Jobs
            </Button>
          </div>
          {jobs.length > 0 ? (
            jobs.map((item, index) => (
              <section key={index} className="shadow-sm single-job row">
                ...
                <div className="content-div col-12 col-sm-12 col-md-9 col-lg-9 col-xl-9">
                  <Link className="link" href={`/JobDetail/${item._id}`}>
                    <h3>{item.title}</h3>
                      ...
                      <>
                        {!renderApplied(item) ? (
                          <Button
                            ...
                            onClick={() => handleJobApply(true, item)}
                            fullWidth
                            className="my-3"
                            variant="contained"
                          >
