...
function Subscription(props) {
  ...
  const getPlans = () => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/candidateSubscription/allSubscription`
      )
      .then((res) => {
        console.log(res);
  ...
  const openPayModal = (amount, plan) => {
    ...
    const options = {
      ...
      handler: function (response) {
        ...
        axios
          .post(
            `${process.env.REACT_APP_DEVELOPMENT}/api/candidate/changeCandidatePlan`,
            { planId: plan._id, plan, paymentId: response },
          ...
          {props.user.userInfo.paymentHistory.length > 0 && plans.length > 0 ? (
            <>
              ...
              <table className="ui celled table">
                ...
                <tbody>
                  {props.user.userInfo.paymentHistory.map((item, index) => (
                    <tr key={index}>
