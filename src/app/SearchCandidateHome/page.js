...
import CandidateCardSmall from "../components/FindCandidates/CandidateCardSmall";
...
function SearchCandidateHome(props) {
  ...
  useEffect(() => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/candidate/getCandidateHomeData`,
        { limit: limit1 },
    ...
    if (error.data === "Unauthorized Access") {
      return (
        <div className="col-12 no-jobs">
          ...
          <Button
            onClick={() => router.push("/Loginrecruiter")}
            variant="contained"
          >
      ...
      <section className="find-cand-home">
        <section className="row m-auto first-cont">
          ...
          <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 text-cont">
            ...
            <p className="desc">
              Welcome to our &quot;Find Candidates&quot; section! Our platform
              is designed to connect you with a diverse pool of talented
              individuals who are actively seeking new job opportunities.
              We&apos;ve curated a database of qualified candidates from a
              variety of industries and backgrounds, so you can easily find the
              perfect fit for your organization. Our user-friendly search tools
              and comprehensive candidate profiles make the hiring process
              simple and streamlined. Sign up today and take your recruitment
              efforts to the next level!
            </p>
            {/* <p className="cand-count"><span className="count">{candidateTotal}+</span> Candidate Data</p> */}
            <Button
              onClick={() => router.push("/searchcandidates")}
              className="mt-3"
        ...
        {data && (
          <section className="small-cand-container">
            <h1 className="px-3">Featured Candidates</h1>
            <div className="row m-auto">
              {data.featuredCandidates.map((item, index) => (
                <div
                  key={index}
                  className="col-12 col-sm-12 col-md-3 col-lg-3 col-xl-3 small-cand-card-parent"
                >
                  <CandidateCardSmall hide={renderHide()} {...item} />
                </div>
              ))}
            </div>
