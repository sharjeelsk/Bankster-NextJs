...
import "../style/SubscriptionR.scss";
...
function Subscription(props) {
  ...
  const openPayModal = (amount, plan) => {
    ...
    const options = {
      ...
      handler: function (response) {
        ...
        axios
          .post(
            `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/changeRecruiterPlan`,
            {
              plan,
              paymentId: response,
              availablePlanCreditsId:
                props.user.userInfo.availablePlanCredits._id,
            },
  ...
  const columns2 = [
    ...
    //{ field: 'brand', headerName: 'Brand Name',valueGetter:(param)=>param.value.name,width:150},
    {
      ...
      valueGetter: (param) => renderPlanName(param.row.subscriptionId),
      width: 150,
    },
    {
      field: "PaymentId",
      headerName: "Payment ID",
      valueGetter: (param) => param.row.paymentId.razorpay_payment_id,
      width: 200,
    },
    {
      field: "expiry",
      headerName: "Expiry",
      valueGetter: (param) =>
        moment.parseZone(param.value).local().format("DD/MM/YY"),
      width: 120,
    },
    {
      field: "createdAt",
        ...
        >
          ...
          {props.user.userInfo.subRecruiter ? (
            <div className="col-12 no-jobs">
              <h1>
                You don&apos;t have access to this section, contact head
                recruiter
              </h1>
            </div>
          ) : (
            <>
              ...
              <p>
                <TaskAltIcon />{" "}
                <b>
                  {props.user.userInfo.availablePlanCredits.cvAccess} resume
                  access left
                </b>
              </p>
              <p>
                <TaskAltIcon />{" "}
                <b>
                  {props.user.userInfo.availablePlanCredits.jobPostings} job
                  postings left
                </b>
              </p>
              <p>
                <TaskAltIcon />{" "}
                <b>
                  {props.user.userInfo.availablePlanCredits.subRecruiters} Sub
                  Recruiters left
                </b>
              </p>
                  ...
                  ? plans.map(
                      (item, index) =>
                        !item.custom && (
                          <div
                            key={index}
                            className={`shadow-sm plan-auth-cont col-12 col-sm-12 col-md-3 col-lg-3 col-xl-3 ${
                              item._id === props.user.userInfo.subscription._id
                                ? "active-plan"
                                : ""
                            }`}
                          >
              ...
              <p><TaskAltIcon /> {item.subRecruiters} Sub Recruiters</p> */}
                            {item._id ===
                            props.user.userInfo.subscription._id ? (
                              <p className="active-text">Currently Active</p>
                            ) : (
                              <Button
                                onClick={() => openPayModal(item.amount, item)}
                                variant="contained"
                              >
                                upgrade
                              </Button>
              ...
              {plans.length > 0 && (
                <div style={{ height: "40vh", width: "100%" }}>
                  <DataGrid
                    rows={props.user.userInfo.paymentHistory.map(
