...
import "../style/FindCandidates.scss";

import { TextField, Button, Slider } from "@mui/material";
import CandidateCardHalf from "../components/FindCandidates/CandidateCardHalf";
...
function SearchCandidates(props) {
  ...
  const [dateFilter, setDateFilter] = useState("All Resume");
  ...
  useEffect(() => {
    if (props.user.userInfo) {
      if (props.user.userInfo.availablePlanCredits) {
        ...
        axios
          .get(
            `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/getAllCandidateLocations`,
        ...
        axios
          .get(
            `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/getAllTags`,
            { headers: { token: props.user.user } }
          )
          .then((res) => {
            // console.log(res);
  ...
  const handleCandidateSearch = (date, refine) => {
    ...
    axios
      .post(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/searchCandidate`,
        obj,
        { headers: { token: props.user.user } }
      )
      .then((res) => {
  ...
  const handleSave = (title) => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/saveSearch`,
        {
          title,
          must,
          mustNot,
      ...
      {error ? (
        ...
        <div className="row m-auto search-candidates-head">
          <section className="col-4 search-candidates shadow-sm">
            <h1>
              Search <span className="primarycolorwh">Candidates</span>
            </h1>
            ...
            <Slider
              onChange={(ev, value) =>
                setFormValues({ ...formValues, minimumAge: value })
              }
            ...
            <p className="slider-info">
              Minimum Age Selected is {formValues.minimumAge} Years
            </p>
            <Slider
              onChange={(ev, value) =>
                setFormValues({ ...formValues, maximumAge: value })
              }
            ...
            />
            <p className="slider-info">
              Maximum Age Selected is {formValues.maximumAge} Years
            </p>
            <hr />
            ...
            />
            <p className="slider-info">
              Minimum Experience Selected is {formValues.minimumExperience}{" "}
              Years
            </p>
            <Slider
              onChange={(ev, value) =>
            ...
            />
            <p className="slider-info">
              Maximum Experience Selected is {formValues.maximumExperience}{" "}
              Years
            </p>
            <hr />
            ...
            />
            <p className="slider-info">
              Minimum Salary Selected is {formValues.minimumSalary} Lacs
            </p>
            <Slider
              onChange={(ev, value) =>
            ...
            />
            <p className="slider-info">
              Maximum Salary Selected is {formValues.maximumSalary} Lacs
            </p>
            <hr />
            ...
            <div className="my-4">
              <Autocomplete
                ...
                getOptionLabel={(option) => option.name}
                renderInput={(params) => (
                  <TextField {...params} label="Select Undergraduate Degree" />
                )}
              />
            </div>
            <div className="my-4">
              <Autocomplete
                ...
                getOptionLabel={(option) => option.name}
                renderInput={(params) => (
                  <TextField {...params} label="Select Post Gradiuate Degree" />
                )}
              />
            </div>
            <hr />
              ...
              <Autocomplete
                ...
                value={
                  formValues.functionalArea
                    ? { name: formValues.functionalArea.name }
                    : { name: "" }
                }
                onChange={(event, newValue) => {
                  if (newValue) {
            ...
            <TextField
              onChange={(e) =>
                setFormValues({ ...formValues, currentCompany: e.target.value })
              }
          ...
          <section className="col-8 candidates-nearby">
            {candidates ? (
              <>
                <h1>
                  Showing{" "}
                  <span className="primarycolorwh">{candidates.length}</span>{" "}
                  Results
                </h1>
                ...
                <div className="menu row m-auto align-items-center">
                  ...
                  <div className="ml-auto">
                    ...
                    >
                      <MenuItem onClick={() => handleClose("All Resume")}>
                        All Resume
                      </MenuItem>
                      <MenuItem onClick={() => handleClose("Last 7 Days")}>
                        Last 7 Days
                      </MenuItem>
                      <MenuItem onClick={() => handleClose("Last 1 Month")}>
                        Last 1 Month
                      </MenuItem>
                      <MenuItem onClick={() => handleClose("Last 6 Months")}>
                        Last 6 Months
                      </MenuItem>
                      <MenuItem onClick={() => handleClose("Last 1 Year")}>
                        Last 1 Year
                      </MenuItem>
                    </Menu>
                  </div>
              ...
              <div>
                <h2>
                  Searched <span className="primarycolorwh">Candidates</span>{" "}
                  will appear here
                </h2>
                <img
                  className="width-100 shadow-sm"
                  src="findcand.png"
                  alt="findcand"
                />
              </div>
