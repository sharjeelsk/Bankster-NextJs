...
function EditProfileImage(props) {
  ...
  const cropImageNow = () => {
    ...
    urltoFile(base64Image, "hello.jpeg", "image/jpeg").then(function (file) {
      console.log(file);
      setOutputFile(file);
  ...
  const handleSubmit = () => {
    ...
    if (props.companyImg) {
      if (outputFile) {
        ...
        axios
          .post(
            `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/upload-recruiter-image`,
            formdata,
            {
              headers: {
          ...
          .then((res) => {
            ...
            props.setSnackbar({
              type: "success",
              text: "Company Image Changed Successfully",
              open: true,
            });
