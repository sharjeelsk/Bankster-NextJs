...
function EditProfile(props) {
  ...
  const [formValues, setFormValues] = useState({ state: "", city: "" });
  const [tags, setTags] = useState([]);
  const [singleTagName, setSingleTagName] = useState("");
  ...
  useEffect(() => {
    ...
    setFormValues({
      state: userInfo.companyLocation.state,
      city: userInfo.companyLocation.city,
    });
  ...
  const onSubmit = (data) => {
    ...
    axios
      .post(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/recruiter/editRecruiterProfile`,
        {
          ...data,
          companyLocation: { ...formValues, country: "India" },
          companyTags: tags,
        },
              ...
              <>
                ...
                <div className="row my-3 mx-auto">
                  <div className="col-10">
                    <TextField
                      fullWidth
                      onChange={(e) => setSingleTagName(e.target.value)}
                      variant="outlined"
                      id="outlined-basic"
                  ...
                  <div className="col-2">
                    <IconButton
                      onClick={() => {
                        if (!tags.includes(singleTagName)) {
                ...
                {tags.map((item, index) => (
                  <Chip
                    ...
                    key={index}
                    label={item}
                    onDelete={() => setTags(tags.filter((i) => i !== item))}
                  />
                ))}
              </>
            }
