  ...
  return (
    ...
    >
      <div className="small-cand-card shadow-sm">
        <h3 className="ctc">
          {props.currentCtc
            ? `₹${numberWithCommas(props.currentCtc)}`
            : "CTC N.A"}
        </h3>
        <img
          src={
            props.profilePicture
              ? `${process.env.REACT_APP_DEVELOPMENT}/api/image/${props.profilePicture}`
              : "/user.png"
          }
          onError={(e) => {
        ...
        <p className="education">
          <b>
            {props.hide
              ? renderEmployementStringHide(props)
              : renderEmployementString(props)}
          </b>
        </p>
        <p className="education">
          <b>
            {props.education.length > 0
              ? props.education.map((i, ind) => {
        ...
        <p className="ginfo">
          {props.gender} | {props.dob ? getAge(props.dob) : "DOB Not Added"}
        </p>
        ...
        <p>
          <b>
            {props.industry ? limitString(props.industry) : "N.A"} |{" "}
            {props.functionalArea ? limitString(props.functionalArea) : "N.A"} |{" "}
            {props.product ? limitString(props.product) : "N.A"}
          </b>
        </p>
        <p className="yoe">{props.yearsOfExperience} Years of Experience</p>
        <p className="yoe">
          {props.noticePeriod
            ? `${props.noticePeriod} Notice Period`
            : "Notice Period N.A"}
        </p>
