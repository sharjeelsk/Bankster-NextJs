...
function JobDetail(props) {
  ...
  useEffect(() => {
      ...
      .then((res) => {
        ...
        if (res.data.msg === "success") {
          ...
          if (props.user.userInfo) {
            if (
              props.user.userInfo.bookmarks.jobs.includes(res.data.result._id)
            ) {
              setBookmarked(true);
            }
          }
          ...
          axios
            .post(
              `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/searchSimilarJobs`,
              {
                functionalArea: res.data.result.functionalArea,
                state: res.data.result.jobLocation.state,
              }
            )
            .then((res) => {
              console.log(res);
    ...
    axios
      .get(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/admin/getFeaturedData`
      )
      .then((res) => {
        console.log(res);
        if (res.data.result.length > 0) {
          setCompanyImg(
            res.data.result.filter((item) => item.type === "companyImg")
          );
  ...
  const findJob = (type, value) => {
    ...
    // if(type==="companyImg"){
    //     axios.post(`${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/searchJobViaCompany`,{companyName:value})
    //     .then(res=>{
    //         console.log(res)
    //         if(res.data.result.length>0){
    ...
    //     })
    // }else if(type==="jobTag"){
    //     axios.post(`${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/searchJobViaTag`,{tags:[value]})
    //     .then(res=>{
    //         console.log(res)
    //         if(res.data.result.length>0){
    //             setJobs(res.data.result)
  ...
  const handleBookmarkAdd = () => {
    if (bookmarked) {
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/candidate/removeBookmarkJob`,
          { jobId: singleJob._id },
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/candidate/bookmarkJob`,
          { jobId: singleJob._id },
          { headers: { token: props.user.user } }
        )
        .then((res) => {
  ...
  const handleJobApply = (apply) => {
    if (apply) {
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/applyJob`,
          { jobId: singleJob._id },
          { headers: { token: props.user.user } }
        )
        .then((res) => {
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/cancelApplyJob`,
          { jobId: singleJob._id },
          { headers: { token: props.user.user } }
        )
        .then((res) => {
  ...
  const renderCancelButton = () => {
    ...
    if (status) {
      ...
      return (
        <Button
          onClick={() => handleJobApply(false)}
          color="error"
          fullWidth
          className="my-3"
          variant="outlined"
        >
          Click to cancel application
        </Button>
      );
    }
  };
    ...
    if (createdBy) {
      if (Array.isArray(createdBy) && createdBy.length > 0) {
        if (createdBy[0].profilePicture.length > 0) {
          return `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/image/${createdBy[0].profilePicture}`;
        } else {
          return "/job-offer.png";
        }
      } else if (createdBy.profilePicture) {
        if (createdBy.profilePicture.length > 0) {
          return `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/image/${createdBy.profilePicture}`;
        } else {
          return "/job-offer.png";
        }
      } else {
  ...
  return (
    <div>
      {/* <Head>
        <title>{singleJob && singleJob.title}</title>
        <meta name="job" content={singleJob && singleJob.jobDescription} />
      </Head> */}
      <Header id="2" />
      <section className="row m-auto find-jobs-head">
        <div className="p-0 col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
          <div className="display-mobile">
            {!display ? (
              ...
              <FilterMenu setDisplay={setDisplay} />
            )}
          </div>
        ...
        <div className=" col-12 col-sm-12 col-md-7 col-lg-7 col-xl-7 find-jobs-content">
          ...
          {singleJob && (
            <section className="shadow-sm single-job row">
              <div className="img-div col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
                <img
                  src={
                    singleJob.createdByAdmin
                      ? renderImageString(singleJob.createdByAdmin)
                      : renderImageString(singleJob.createdBy)
                  }
                  alt="logo1"
                />
              </div>
              <div className="content-div col-12 col-sm-12 col-md-9 col-lg-9 col-xl-9">
                ...
                <div className="button-div">
                  {props.user.userType === 0 ? (
                    <>
                      {!renderApplied() ? (
                        <Button
                          disabled={checkDisabled(singleJob)}
                          onClick={() => handleJobApply(true)}
                          fullWidth
                          className="my-3"
                          variant="contained"
                        >
                ...
                <div className="row my-2 mx-auto key-features">
                  ...
                  <div className="m-1">
                    <CurrencyRupeeIcon />
                    <span className="key-headline m-2">
                      {singleJob.ctc.min} - {singleJob.ctc.max} P.A
                    </span>
                  </div>
                  <div className="m-1">
                    <FmdGoodIcon />
                    <span className="key-headline m-2">
                      {singleJob.jobLocation.city} |{" "}
                      {singleJob.jobLocation.state} |{" "}
                      {singleJob.jobLocation.country}
                    </span>
                  </div>
                </div>
                <div className="row my-2 mx-auto key-features">
                  <div className="m-1">
                    <ArticleIcon />
                    <span className="key-headline m-2">
                      {singleJob.qualification.ug}
                    </span>
                  </div>
                  <div className="m-1">
                    <DescriptionIcon />
                    <span className="key-headline m-2">
                      {singleJob.qualification.pg}
                    </span>
                  </div>
                  <div className="m-1">
                    <Inventory2Icon />
                ...
                <div className="description">
                  ...
                  <ul>
                    {singleJob.roleResp.map((item, index) => (
                ...
                <div className="description">
                  ...
                  <div className="recruiter-info shadow-sm row m-auto align-items-center">
                    <div className="col-2 img-div">
                      <img
                        src={
                          singleJob.createdByAdmin
                            ? renderImageString(singleJob.createdByAdmin)
                            : renderImageString2(singleJob.createdBy)
                        }
                        alt="logo1"
                      />
                    </div>
                    <div className="col-8 content-div">
                      <h3>
                        {singleJob.createdByAdmin
                          ? renderRecruiterName(singleJob.createdByAdmin)
                          : renderRecruiterName(singleJob.createdBy)}
                      </h3>
                      <p style={{ margin: 0 }}>
                        {singleJob.createdByAdmin
                          ? renderRecruiterCompany(singleJob.createdByAdmin)
                          : renderRecruiterCompany(singleJob.createdBy)}
                      </p>
                      <p style={{ margin: 0 }}>
                        {singleJob.createdByAdmin
                          ? renderRecruiterCompanyDescription(
                              singleJob.createdByAdmin
                            )
                          : renderRecruiterCompanyDescription(
                              singleJob.createdBy
              ...
              <section key={index} className="shadow-sm single-job row">
                ...
                <div className="content-div col-12 col-sm-12 col-md-8 col-lg-8 col-xl-8">
                  ...
                  >
                    ...
                    <div className="keys">
                      {item.tags.map((tag, index) => (
                        <Chip key={index} className="m-3" label={tag} />
                      ))}
                    </div>
                  </Link>
                  ...
                  {/* <div className="button-div">
                            ...
                            <>
                            <Button fullWidth onClick={()=>router.push("/login")} className="my-2" variant='outlined'>Log in to apply</Button>
                            <Button fullWidth onClick={()=>router.push("/signup")} className="my-2" variant='contained'>Register to apply</Button>
                            </>
                            }
                        </div> */}
                </div>
                <div className="bookmark-div col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
                  ...
                  >
                    {props.user.user ? (
                      props.user.userInfo.bookmarks.jobs.includes(item._id) ? (
                        <BookmarkIcon />
                      ) : (
                        <BookmarkBorderIcon />
                      )
        ...
        <div className=" col-12 col-sm-12 col-md-3 col-lg-3 col-xl-3 right-section-job">
          <section className="shadow-sm search-container">
            <h4>
              <SearchIcon /> Search Jobs Via Company
            </h4>

            {companyImg &&
              companyImg.map((item, index) => (
                <img
                  onClick={() => findJob(item.type, item.name)}
                  key={index}
          ...
          <section className="shadow-sm search-container">
            <h4>
              <SearchIcon /> Search Jobs Via Tags
            </h4>
