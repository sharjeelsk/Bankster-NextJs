...
import ChangeJobStatus from "../../components/JobsCreated/ChangeJobStatus";
...
function RecruiterJobDetail(props) {
  ...
  const handleHire = (candidateObjectId, userId) => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/hireCandidate`,
        {
          jobId: singleJob._id,
          candidateObjectId,
          userId,
  ...
  const handleReject = (candidateObjectId, userId) => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/rejectCandidate`,
        {
          jobId: singleJob._id,
          candidateObjectId,
          userId,
  ...
  const handleShortlist = (candidateObjectId, userId) => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/job/shortlistCandidate`,
        {
          jobId: singleJob._id,
          candidateObjectId,
          userId,
          ...
          <TabContext value={value}>
                ...
                <section className="shadow-sm single-job row">
                  ...
                  <div className="content-div col-12 col-sm-12 col-md-9 col-lg-9 col-xl-9">
                    ...
                    <p className="company-name m-0">
                      {singleJob.createdByAdmin
                        ? singleJob.createdByAdmin.companyName
                        : singleJob.createdBy.companyName}
                    </p>
                    ...
                    <div className="row my-2 mx-auto key-features">
                      <div className="m-1">
                        ...
                        <span className="key-headline m-2">
                          {singleJob.experience.min} -{" "}
                          {singleJob.experience.max} Yrs
                        </span>
                      </div>
                      <div className="m-1">
                        <CurrencyRupeeIcon />
            ...
            <TabPanel value="2">
              {singleJob && singleJob.jobCandidates.length > 0 ? (
                <>
                  <SearchBar2
                    searchText="Search By Applicants Name..."
                  ...
                  {filteredCandidates.length > 0 ? (
                    filteredCandidates.map((item, index) =>
                      !item.user ? (
                        <>
                          <h3>
                            {index + 1} Sorry, this applicant with status&quot;
                            {item.status}&quot; doesn&apos;t exist in our system
                          </h3>
                        </>
                      ) : (
                        <section
                          className="candidate-single-job shadow-sm"
                          key={index}
                        >
                          <div className="row m-auto">
                              ...
                              <img
                                src={
                                  item.user.profilePicture.length <= 0
                                    ? "/user.png"
                                    : `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/image/${item.user.profilePicture}`
                                }
                                alt="logo1"
                              />
                            </div>
                            <div className="col-9 col-sm-9 col-md-7 col-lg-7 col-xl-7">
                              ...
                              <p className="bold-text">
                                {renderEmployementString(item.user)}
                              </p>
                              <p className="bold-text">
                                {item.user.education.length > 0
                                  ? item.user.education.map((i) => {
                                      if (i.featuredEducation) {
                                        return i.name + ", " + i.universityName;
                                      }
                                    })
                                  : "User hasn't added featured education"}
                              </p>
                              <p className="grey-text">
                                {item.user.gender} |{" "}
                                {item.user.dob
                                  ? getAge(item.user.dob)
                                  : "DOB Missing"}
                              </p>
                              <p className="grey-text">
                                {item.user.yearsOfExperience
                                  ? item.user.yearsOfExperience
                                  : "Not added"}{" "}
                                Years of Experience |{" "}
                                {item.user.currentCtc
                                  ? item.user.currentCtc
                                  : "Not added"}{" "}
                                CTC |{" "}
                                {item.user.product
                              ...
                              <p className="bold-text">
                                {item.user.education.length > 0
                                  ? `${item.user.education[0].name}, ${item.user.education[0].universityName}`
                                  : ""}
                              </p>
                              <div className="row mt-4 mx-auto sub-info">
                                <p className="mx-2">
                                  <LocalPhoneIcon sx={{ marginRight: 0.1 }} />{" "}
                                  <b>{item.user.mobileNo}</b>
                                </p>
                                <p className="mx-2">
                                  <AlternateEmailIcon
                                    sx={{ marginRight: 0.1 }}
                                ...
                                <p className="mx-2">
                                  ...
                                  <b>
                                    {item.user.userLocation.city},{" "}
                                    {item.user.userLocation.state}
                                  </b>
                                </p>
                              </div>
                              <p className="tagline">
                                {item.user.resumeTagline}
                              </p>
                            </div>
                            <div className="col-10 col-sm-10 col-md-3 col-lg-3 col-xl-3">
                              {item.user.resume.length > 0 && (
                                <div
                                  onClick={() => {
                                    window.open(
                                      `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/pdf/${item.user.resume}`,
                                      "_blank"
                                    );
                                    axios
                                      .post(
                                        `${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/candidate/increaseProfileCount`,
                                        { candidateId: item.user._id },
                                        { headers: { token: props.user.user } }
                                      )
                                      .then((res) => {
                                        console.log(res);
                                    ...
                                    //window.location.href=`${process.env.NEXT_PUBLIC_REACT_APP_DEVELOPMENT}/api/pdf/${item.user.resume}`
                            ...
                            <div
                              className="col-1 col-sm-1 col-md-1 col-lg-1 col-xl-1 bookmark-div"
                            ...
                            {item.status === "Pending" ? (
                              <>
                                <Button
                                  color="error"
                                  onClick={() =>
                                    handleReject(item._id, item.user._id)
                                  }
                                >
                                  Reject
                                </Button>
                                ...
                                <Button
                                  color="info"
                                  onClick={() =>
                                    handleShortlist(item._id, item.user._id)
                                  }
                                >
                                  Shortlist
                                </Button>
                                ...
                                <Button
                                  onClick={() =>
                                    handleHire(item._id, item.user._id)
                                  }
                                  variant="contained"
                                >
                                  Hire
                              ...
                              <>
                                <span className={renderStatus(item.status)}>
                                  {item.status}
                                </span>
                                <br />
                                <Button
